...
import { type RouteDetails } from './find-routes'
import { type RouteData } from '../lib/router'
import { createImport } from '../utils/import'
import { type MiddlewareData, createMiddlewares } from './create-middlewares'
...
function createRoutesContent(routes: RouteData[], imports: string[]) {
	...
	const routesString = JSON.stringify(routes).replace(/"IMPORT\{([^"]+)\}"/g, '$1')
